...
page = requests.get('https://status.zoom.us/', headers=headers)

soup = BeautifulSoup(page.text, 'html.parser')

divs =  soup.find_all("div", {"class": "component-container border-color"})

for div in divs:
    service_name = div.find('span',{"class": "name"}).text.replace("\n","").strip()
    service_status = div.find('span',{"class": "component-status"}).text.replace("\n","").strip()
    ...
    if service_status != "Operational":
        status.append(f"{service_name}: Fail")
        problems = problems + 1
        perfdata = perfdata + service_name.lower().replace(' ','_') + "=1 "
    else:
        status.append(f"{service_name}: OK")
        perfdata = perfdata + service_name.lower().replace(' ','_') + "=0 "
